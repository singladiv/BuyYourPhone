...
const [id, setId] = useState();
  ...
  const order = {
    device_name: selectedVariant?.model+" "+selectedVariant?.model,
    device_variant : vrnt,
    device_price : price,
    device_img: selectedVariant?.image,
  }
  ...
  const onSubmit = async (e) => {
    e.preventDefault();
    await axios.post("http://localhost:8080/api/buyphone/createOrder",order).then((res)=>{
